...
class PandaEnv(gym.Env):
    ...
    def reset(self):
        ...
        # self.save_image(goal=True)

        self.reset_object()
        self.object_rearrange()
        init_img = self.get_image()
        # self.save_image(action=14)
    ...
    def select_task(self, pred_task):
        
        possible_action = self.get_possible_action()

        if pred_task in possible_action:
            self.success += 1
            return pred_task
        else:
    ...
    def high_step(self, task_num):
        ...
        print(f'{self.success/self.trial}')
        
        high_next_o, high_r, high_d = self.get_high_state(task_num)

        return high_next_o, high_r, high_d
    ...
    def low_step_script(self, task_num):
        ...
        self.robot_move(a)
        low_next_o, low_r, low_d = self.get_low_state(task_num=task_num, action=a)
        self.low_steps += 1
    ...
    def get_low_state(self, task_num=14, action=None):
        ...
        if task_num != 10:		
            done = self.done_list[task_num]()
            if done and action[5] > 0.5:
                reward = 1.0
                # print("OH YEAH")
    ...
    def get_image(self):
        ...
        # self.rgb = rgb[20:20+216,20:20+216,:]
    ...
    def reset_object(self):
        ...
        if self.green_goal_location == 0:
            ...
            if self.green_cur_location == 0:
                ...
                self.green_pos += np.array([random.uniform(-0.03, 0.03), random.uniform(-0.03, 0.03), 0.05])
        ...
        if self.orange_goal_location == 0:
            ...
            if self.orange_cur_location == 0:
                ...
                self.orange_pos += np.array([random.uniform(-0.03, 0.03), random.uniform(-0.03, 0.03), 0.05])
    ...
    def goal_object(self):
        ...
        self.laptop_pos = np.array([0.45+random.uniform(-0.03, 0.018),0.41+random.uniform(-0.03, 0.03),0.0])
        ...
        # Drawer pose
        self.drawer_pos = np.array([0.70+random.uniform(-0.018, 0.03),0.38+random.uniform(-0.03, 0.03), -0.002])
        ...
        if self.box_goal_location == 0:
            self.box_pos = np.array([0.48,-0.15+random.uniform(-0.03, 0.03),0.0]) #+random.uniform(-0.03, 0.03)
            ...
            self.box_pos = np.array([0.7,-0.15+random.uniform(-0.03, 0.03),0.0]) #+random.uniform(-0.03, 0.03)
            self.box_ori = random.uniform(-0.05, 0.05)
        ...
        self.cover_ori = self.box_ori
        self.coverStartOrientation = p.getQuaternionFromEuler([0.0 ,0, self.cover_ori])
        self.cover_open_pos, self.cover_open_ori = np.array([0.7, -0.15, 0.07]), 0.0
