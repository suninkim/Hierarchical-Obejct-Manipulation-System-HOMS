...
class PandaEnv(gym.Env):
    ...
    def __init__(self, args, gui):
        ...
        self.high_observation_space = gym.spaces.Box(0, 255, (6,216,216))
        self.high_action_space = gym.spaces.Box(0, 1.0, (self.num_task,))
        self.low_observation_space = gym.spaces.Box(0, 255, (3,216,216))
    ...
    def action_definition(self):
        ...
        self.task_step = np.zeros(self.num_task)
        self.img_num = 2000
        self.ignore_act = [10]
    ...
    def reset(self):
        ...
        self.reset_object()
        self.object_rearrange()
        init_img = self.get_image()

        simulator.step_simulation(self.num_sim_steps_reset)

        high_state = {'image': init_img, 'goal': self.goal_img}
    ...
    def step(self):
        ...
        posb_task = np.zeros(self.num_task)
        act_task = np.zeros(self.num_task)

        task_num = possible_action[random.randint(0, len(possible_action)-1)]
        posb_task[possible_action] = 1
        act_task[task_num] = 1
        ...
        if task_num in [0]:
            inner_step = 20
        elif task_num in [5, 1, 2]:
            inner_step = 25
        elif task_num in [6,8]:
            inner_step = 35
        else:
            inner_step = 30
        self.low_step(task_num, inner_step=inner_step)
        ...
        high_state, high_reward, high_done = self.get_high_state(task_num)

        return high_state, high_reward, high_done, act_task, possible_action
    
    def low_step(self, task_num, inner_step=30):

        self.robot_action_make(task_num)
        self.cur_point = 0

        task = np.zeros(self.num_task)
        task[task_num] = 1
        ...
        d = False
        self.set_color()
        o, _, _ = self.get_low_state()
        next_o = None        

        robot_step =0 
        while robot_step< inner_step and not d:
            if task_num in self.ignore_act:
                break
            elif task_num == 0:
                a, self.cur_point = task_policy.make_action_laptop_close(
                                        self.robotId, self.pre_pos, self.pre_ori, self.tar_pos, self.tar_ori, self.is_gripper_open, self.cur_point)
            elif task_num == 1:
                a, self.cur_point = task_policy.make_action_drawer_open(
                                        self.robotId, self.tar_pos, self.tar_ori, self.tar_vec, self.is_gripper_open, self.cur_point)
            elif task_num == 2:
                a, self.cur_point = task_policy.make_action_drawer_close(
                                        self.robotId, self.tar_pos, self.tar_ori, self.tar_vec, self.is_gripper_open, self.cur_point)
            elif task_num == 5:
                a, self.cur_point = task_policy.make_action_box_push(
                                        self.robotId, self.tar_pos, self.tar_ori, self.tar_vec, self.is_gripper_open, self.cur_point) 
            elif task_num in [6,8]:
                a, self.cur_point = task_policy.make_action_laptop_grasp(
                                        self.robotId, self.tar_pos, self.tar_ori, self.drop_pos, self.drop_ori, self.is_gripper_open, self.cur_point)      
            else:
                a, self.cur_point = task_policy.make_action(
                                        self.robotId, self.tar_pos, self.tar_ori, self.drop_pos, self.drop_ori, self.is_gripper_open, self.cur_point)

            self.robot_move(a)
            next_o, r, d = self.get_low_state(task_num=task_num, action=a)
            observations.append(o)
            actions.append(a)
            rewards.append(r)
            terminals.append(d)
            tasks.append(task_num)
            self.task_step[task_num] += 1
            robot_step +=1
    ...
    def get_low_state(self, task_num=10, action=None):
        ...
        return {'image': image, 'robot': robot_state}, reward, done
    ...
    def d_act3(self):
        ...
        now_open_dist = self.cal_distance(self.cover_open_pos, cover_pos)
    ...
    def camera_setting(self):
        ...
        # self.cam_pos = np.array([-0.032,  0.208,  0.328])
        # self.cam_tar = np.array([ 0.288,  0.208,  0.213])
    ...
    def index(self):
        ...
        for i in range(2):
            self.green_index.append(self.greenId+((i)<<24))   

        for i in range(2):
            self.orange_index.append(self.orangeId+((i)<<24))
    ...
    def set_color(self):
        ...
        for i in range(11):
            ...
            change = np.random.uniform(-0.2, 0.0, 4)
            change[3] = 0
            color += change
            self.robot_color.append(color)
            p.changeVisualShape(self.robotId, i, rgbaColor=color)

        # Plane color
        change = np.random.uniform(-0.8,0.05,4)
        change[3] = 0
        color = np.array([0.95,0.95,0.95,1])
        color += change
        self.plane_color = color
        p.changeVisualShape(self.planeId,-1, rgbaColor=color)

        # Wall_color
        change = np.random.uniform(-0.2,0.1,4)
        change[3] = 0
        ...
        # Table_color
        change = np.random.uniform(-0.2,0.2,4)
        change[3] = 0
        color = np.array([0.9,0.8,0.65,1])
        color = np.clip(color+change, 0.0, 1.0)
        ...
        for i in range(2):
            p.changeVisualShape(self.laptopId,i-1, rgbaColor=color)

        # Drawer_out_color
        change = np.random.uniform(-0.2,0.2,4)
        change[3] = 0
        color = np.array([0.0, 0.522, 0.565,1])
        color = np.clip(color+change, 0.0, 1.0)
        self.drawer_out_color = color
        ...
        change = np.random.uniform(-0.2,0.2,4)
        change[3] = 0
        color = np.array([0.792, 0.047, 0.047,1])
        color = np.clip(color+change, 0.0, 1.0)
        self.drawer_in_color = color
        ...
        change = np.random.uniform(-0.2,0.2,4)
        change[3] = 0
        color = np.array([0.879, 0.719, 0.125,1])
        color = np.clip(color+change, 0.0, 1.0)
        self.drawer_handle_color = color
        for i in self.drawer_handle_list:            
            p.changeVisualShape(self.drawerId, i, rgbaColor=color)

        # Tray_color
        change = np.random.uniform(-0.15,0.15,4)
        change[3] = 0
        color = np.array([0.722, 0.757, 0.780,1])
        color = np.clip(color+change, 0.0, 1.0)
        self.tray_color = color
        for i in range(6):            
            p.changeVisualShape(self.trayId, i-1, rgbaColor=color)
        ...
        for i in self.box_inner:            
            p.changeVisualShape(self.boxId,i, rgbaColor=color)
        for i in self.cover_inner:          
            p.changeVisualShape(self.coverId,i, rgbaColor=color)

        # Box_outer_color
        change = np.random.uniform(-0.2,0.2,4)
        change[3] = 0
        color = ([0.106, 0.463, 0.720, 1])
        color = np.clip(color+change, 0.0, 1.0)
        self.box_out_color = color
        ...
        change = np.random.uniform(-0.2,0.2,4)
        change[3] = 0
        color = np.array([0.957, 0.443, 0.710, 1])
        color = np.clip(color+change,0.0,1.0)
        self.cover_out_color = color
        ...
        change = np.random.uniform(-0.2,0.2,4)
        change[3] = 0
        color = ([0.973, 0.741, 0.067, 1])
        color = np.clip(color+change,0.0,1.0)
        self.cover_handle_color = color
        p.changeVisualShape(self.coverId,9, rgbaColor=color)

        # Green_color
        change = np.random.uniform(-0.13,0.13,4)
        change[3] = 0
        color = ([0.753, 0.847, 0.0, 1.0])
        color = np.clip(color+change, 0.0, 1.0)
        self.green_color = color
        p.changeVisualShape(self.greenId,-1, rgbaColor=color)

        # Orange_color
        change = np.random.uniform(-0.13,0.13,4)
        change[3] = 0
        color = ([0.831, 0.271, 0.094, 1.0])
        color = np.clip(color+change, 0.0, 1.0)
        self.orange_color = color
        p.changeVisualShape(self.orangeId,-1, rgbaColor=color)        

    def get_image(self, task_num=10):
        ...
        if task_num != 10:
            ...
            rand_pos = np.random.uniform(-0.03, 0.03, 3)
            rand_tar = np.random.uniform(-0.03, 0.03, 3)
        ...
        images = p.getCameraImage(self.width,self.height,self.view_matrix,self.projection_matrix, shadow=True, lightColor=Color, lightAmbientCoeff=AmbientCoeff, lightDirection=direction, lightDiffuseCoeff=DiffuseCoeff, lightSpecularCoeff=SpecularCoeff)
        rgb = np.reshape(images[2], (self.height, self.width, 4))[:,:,:3]
        seg, dep = np.reshape(images[4], (self.height, self.width, )),np.reshape(images[3], (self.height, self.width, )) 
        seg, dep = self.seg_dep_get(seg, dep)
        can = rgb

        if task_num != 10:
            ori = ori[6:6+240, 12:12+240,:].astype(np.uint8)
            can = can[6:6+240, 12:12+240,:].astype(np.uint8)
            seg = seg[6:6+240, 12:12+240,:].astype(np.uint8)
            dep = dep[6:6+240, 12:12+240,:].astype(np.uint8)

            save_num = int(task_num*self.img_num + self.task_step[task_num])
            if self.task_step[task_num]> self.img_num:
                self.ignore_act.append(task_num)
            if save_num < (task_num+1)*self.img_num:
                np.save(f"rcan_data/ori/{save_num}.npy",ori)
                np.save(f"rcan_data/can/{save_num}.npy",can)
                np.save(f"rcan_data/seg/{save_num}.npy",seg)
                np.save(f"rcan_data/dep/{save_num}.npy",dep)

        self.rgb = can[6:6+240, 12:12+240,:]
    ...
    def seg_dep_get(self, seg, dep):
        ...
        seg_img = np.zeros((self.height, self.width, 10))
        ...
        for ind in self.green_index:
            seg_img[seg == ind,8] = 255

        for ind in self.orange_index:
            seg_img[seg == ind,9] = 255
    ...
    def canonical_color_definition(self):
        ...
        self.PLANE_COLOR = [0.9, 0.9, 0.9, 1]
    ...
    def canonical_color(self):
        for i in range(2):
            p.changeVisualShape(self.planeId,i-1, rgbaColor=self.PLANE_COLOR)
    ...
    def reset_color(self):
        ...
        for i in range(11):
            p.changeVisualShape(self.robotId, i, rgbaColor=self.robot_color[i])

        # Plane
        p.changeVisualShape(self.planeId,-1, rgbaColor=self.plane_color)
    ...
    def goal_object(self):
        ...
        self.cover_ori = self.box_ori
        self.coverStartOrientation = p.getQuaternionFromEuler([0.0 ,0, self.cover_ori])
        self.cover_open_pos, self.cover_open_ori = np.array([0.7+random.uniform(-0.03, 0.03), -0.15+random.uniform(-0.03, 0.03), 0.07]), random.uniform(-0.05, 0.05)
