...
class PandaEnv(gym.Env):
    ...
    def action_definition(self):
        ...
        for _ in range(self.num_task):
            self.rollout.append([])
        self.saved_num = np.zeros(self.num_task)
        self.saved_task = np.ones(self.num_task)
        self.success = np.zeros(self.num_task)
        self.non_success = np.zeros(self.num_task)
        self.saved_task_num = np.zeros(self.num_task)

        self.ignore_act = [10]
        self.total_save_rollout = 10
        self.num_epi_per_rollout = 100
        self.non_success_num = 40
        self.success_num = self.num_epi_per_rollout- self.non_success_num
    ...
    def low_step(self, task_num, inner_step=30):
        ...
        if task_num not in self.ignore_act and self.saved_task_num[task_num] < self.total_save_rollout:
            ...
            if d and self.success[task_num] < self.success_num:
                self.rollout[task_num].append(epi_dict)
                self.saved_num[task_num] += 1
                self.saved_task[task_num] = False
                self.success[task_num] += 1

            elif not d and self.non_success[task_num] < self.non_success_num:
                self.rollout[task_num].append(epi_dict)
                self.saved_num[task_num] += 1
                self.saved_task[task_num] = False
                self.non_success[task_num] += 1

            if self.saved_num[task_num] % self.num_epi_per_rollout == 0 and not self.saved_task[task_num]:
                save = np.array(self.rollout[task_num])
                np.save(f'rollout_F1/task{task_num}/rollout{int(self.saved_num[task_num] / self.num_epi_per_rollout)}.npy',save)
                self.saved_task[task_num]=True
                self.non_success[task_num] = 0
                self.success[task_num] = 0
                self.rollout[task_num] = []
                self.saved_task_num[task_num] += 1
                print(f'task{task_num} rollout{int(self.saved_num[task_num] / self.num_epi_per_rollout)} is saved')
                if self.saved_task_num[task_num] == self.total_save_rollout:
    ...
    def save_image(self, goal=False, action=None):
        ...
        rand_pos = np.random.uniform(-0.01, 0.01, 3)
        rand_tar = np.random.uniform(-0.01, 0.01, 3)
    ...
    def seg_dep_get(self, seg, dep):
        ...
        seg_img = np.zeros((self.height, self.width, 7))
